...
assessmentButton.addEventListener(//イベント検知設定の追加
  ...
  () => { //無名関数でアロー関数、イベント検知時のみ実行
    ...
    resultDivision.innerText = '' //divタグを空文字で上書きしている。
    const header = document.createElement('h3'); //h3タグの作成
    header.innerText = '診断結果'; //タグの内側のテキストを設定
    resultDivision.appendChild(header); //divタグの子要素として設定
    ...
    //ツイートエリアの作成
     ...
     anchor.setAttribute ('href', herfValue) //属性hrefを追加
     anchor.setAttribute ('class', 'twitter-hashtag-button')
     anchor.setAttribute ('data-text', result + '#あなたのいいところ') //診断結果を追加
     ...
     tweetDivision.appendChild(script);//divの子要素として累加
  }
);
